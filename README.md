...
- 行情与财务数据在浏览器里从东方财富获取（每只股票只取成分期前后的区间），存进本机 IndexedDB，可暂停、下次接着取；财务数据用"主要指标"接口（含已退市公司），公告日的下一个交易日起才可用，滚动 12 个月每股收益只用当时已公告的报告计算
...
- 个股池只有沪深300；已退市公司能否取到行情取决于东方财富，取不到的会在页面上列为缺失；财务数据遇到更正公告时按数据源的最新值计，存在轻微前视
