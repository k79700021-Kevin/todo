...
- 多因子选股：多个因子在标的之间排名打分、按权重合成，每期买入前 N 只，可加大盘趋势过滤；仓位可选"组合优化"：候选为综合分最高的 max(3×N, 30) 只加现有持仓，Grinold 预期收益 α = IC × σ × z，协方差用过去 250 日收益的 Ledoit–Wolf 收缩估计（向常相关），在单票上限、满仓、换手惩罚（单边成本）与可选的行业偏离惩罚下做均值-方差优化（FISTA 近端梯度，换手与上限的近端算子精确求解）；因子库含动量、反转、均线偏离、RSI、MACD、布林 %B、KDJ、波动率、ATR、量比，以及距 250 日新高、20 日最大单日涨幅（彩票效应）、Amihud 非流动性；个股池另有盈利收益率 E/P（TTM）、账面市值比 B/P、ROE（TTM）、净利润与营收同比增速、流通市值
- 另有 ETF 动量轮动与买入持有
...
- 最新信号：每只标的当前持仓、各规则的多空状态、下一交易日开盘要做的操作；可"冻结今日信号"作为影子组合：目标仓位连同策略与数据哈希写入研究账本，之后更新行情时按下一交易日开盘买入、逐日结算，积累真正的样本外记录
...
- Hansen SPA 检验：全部参数组合相对基准的日超额收益，平稳自助法；回答"试了这么多组之后，最好的那组是否真的优于基准"
- 研究账本（`docs/ledger.js`）：只追加、带 SHA-256 哈希链；每个不同的回测配置（含配置与数据指纹、结果摘要）、每次优化（含 DSR / PBO / SPA）、每次查看测试集、每次解锁最终留出期、每次冻结信号都会记入；改动或删除任何一条校验时都会暴露；可导出备份、在其他设备导入合并（每台设备一条链）
