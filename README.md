...
- 风控与持仓：止损、止盈、ATR 移动止损、最多持有几只（信号多于仓位时按近 20 日涨幅择优）、最短持有天数、最长持有天数（时间止损）、平仓后冷却天数；仓位可按 1/N 固定、持仓等分或波动率倒数分配
...
- 另有 ETF 动量轮动与买入持有
- 评价：收益/风险指标；相对基准的 α（CAPM）、β、信息比率、跟踪误差；按笔统计的胜率、盈亏比、利润因子、平均持有天数；月度收益表
...
- 滚动前推（walk-forward）：只在训练+验证段内进行，每个窗口只用之前的数据选参，拼接各检验窗口得到样本外净值
- 通缩夏普比率（Bailey & López de Prado）：按试验次数折算，给出训练集第一名"真实夏普 > 0"的概率
- 参数热力图：训练集或验证集，区分稳定的参数区域和孤立的过拟合尖峰

**时序与因子**
- 收益序列：偏度、超额峰度、自相关（含绝对收益的波动聚集）、Lo–MacKinlay 方差比检验
- 16 个因子的时序 IC 与截面 IC（非重叠抽样，带 t 值），分组收益，并提示多重检验下的偶然显著数
